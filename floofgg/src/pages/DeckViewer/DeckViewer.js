...
const DeckViewer = () => {
  ...
  return (
    <div>
      ...
      <h2>Main Deck</h2>
      <div className="deck" id="mainDeck">
        {mainDeck.map(cardId => (
          <div key={cardId} className="card">
            <img src={`https://images.ygoprodeck.com/images/cards/${cardId}.jpg`} alt={`Card ${cardId}`} />
            <div className="tooltip">Card ID: {cardId}<br />Value: {cardValues[cardId]}</div>
          </div>
        ))}
      </div>
      <h2>Extra Deck</h2>
      <div className="deck" id="extraDeck">
      ...
      <h2>Side Deck</h2>
      <div className="deck" id="sideDeck">
        {sideDeck.map(cardId => (
          <div key={cardId} className="card">
            <img src={`https://images.ygoprodeck.com/images/cards/${cardId}.jpg`} alt={`Card ${cardId}`} />
            <div className="tooltip">Card ID: {cardId}<br />Value: {cardValues[cardId]}</div>
      ...
      <h2>Test Hand</h2>
      <div className="deck" id="testHand">
        {testHand.map(cardId => (
          <div key={cardId} className="card">
            <img src={`https://images.ygoprodeck.com/images/cards/${cardId}.jpg`} alt={`Card ${cardId}`} />
          </div>
